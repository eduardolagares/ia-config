...
name: comitar
description: >-
  Diff/staging → mensagem curta pt-BR → git add + commit sem pedir confirmação.
  Use com /comitar, "comitar", "fazer commit", ou quando o utilizador pedir commit
  das alterações atuais.
...
**Somente esta skill:** ao executar `comitar` ou `/comitar`, **não** ler, invocar ou seguir nenhuma skill ou regra externa de mensagem de commit (ex.: caveman-commit, `/caveman-commit`, plugins de commit). O fluxo e o formato estão **só** neste arquivo.
