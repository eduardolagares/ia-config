...
**Protocolo:** aplicar integralmente [../code-review/SKILL.md](../code-review/SKILL.md) (`/code-review`).
...
Cada achado `N.M` nos blocos **1–4** deve referir linha/path **presente no diff do passo 3**.
...
| **1 projeto** | Pode omitir headings `###` — itens direto sob `1 - Crítico` / `2 - Grave` |
| **2+ projetos** | **Obrigatório** agrupar: sob cada bloco de severidade, um `### <repo>` antes dos itens daquele repo |
| **Repo sem achados** | Incluir `### <repo>` com `Nenhum.` (Crítico/Grave) — prova que o projeto foi revisado |
| **Repo só com Erro:** no diff | `### <repo>` + linha explicando indisponibilidade; blocos 1–2 `Nenhum.` para esse repo |
...
Sem diff com conteúdo (só erros por repo) → entregar code review com stocks do bloco 4; não inventar achados.
...
4. Ler [../code-review/SKILL.md](../code-review/SKILL.md) e seguir **todas** as regras (read-only, pt-BR, blocos 1–4).
...
8. **Bloco 4:** lacunas **por projeto** — comparar diff ↔ testes de cada repo (sem rodar suite, sem cobertura %).
...
|-----------------|----------------|
| **1 - Crítico** (`1.M`) | Tópico **`## Revisão de código`** no doc Monday — checkbox `- [ ]`, **agrupado por `### <repo>`** |
| **2 - Grave** (`2.M`) | Idem |
| **3 - Outros** | Não publicar |
| **4 - Lacunas** | Não publicar (salvo pedido explícito) |

Passo 6: [gerar-requisitos-de-codigo/SKILL.md](../gerar-requisitos-de-codigo/SKILL.md) — publica **1.M** e **2.M** no tópico **Revisão de código** (Monday; cria doc se ausente).
...
| Diff só com **Erro:** por repo | Code review por repo com erro; blocos 1–4 `Nenhum.` ou stock bloco 4 |
