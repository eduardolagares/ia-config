#!/usr/bin/env bash
# Passo 3 /revisar-tarefa: diff da branch vs master por projeto (glab).
# Uso: glab-phase3-diff-bundle.sh <source-branch> <repo1> [repo2 ...]
# Saída: JSON no stdout (metadados + paths dos diffs em /tmp).
set -euo pipefail

BRANCH="${1:?Uso: glab-phase3-diff-bundle.sh source-branch repo [repo2...]}"
shift
...
for repo in "${REPOS[@]}"; do
  ...
  set +e
  find_out="$( "${SCRIPT_DIR}/glab-mr-find.sh" "${repo}" "${BRANCH}" 2>&1)"
  find_ec=$?
  set -e
  ...
  if [[ "${find_ec}" -eq 2 ]]; then
    ...
    results+=("$(jq -n \
      ...
      --arg branch "${BRANCH}" \
      --arg base "${BASE_REF}" \
      --arg err "GLAB_RUN_IN_USER_TERMINAL" \
  ...
  if [[ "${find_ec}" -eq 0 ]] && [[ "${count}" -gt 0 ]]; then
    ...
    set +e
    if "${SCRIPT_DIR}/glab-mr-diff.sh" "${repo}" "${mr_iid}" "${diff_file}" >/dev/null 2>&1; then
      method="mr_diff"
    else
      mr_diff_ec=$?
      if [[ "${mr_diff_ec}" -eq 2 ]]; then
        ...
        err_msg="glab mr diff falhou (iid=${mr_iid})"
      fi
    fi
    set -e
  fi
  ...
  if [[ -z "${method}" && -z "${err_msg}" ]]; then
    set +e
    if "${SCRIPT_DIR}/glab-compare-diff.sh" "${repo}" "${BRANCH}" "${BASE_REF}" "${diff_file}" >/dev/null 2>&1; then
      method="compare"
      ...
      if [[ "${cmp_ec}" -eq 2 ]]; then
        ...
        err_msg="sem MR na branch e compare falhou"
      else
        err_msg="compare ${BASE_REF}...${BRANCH} falhou"
      fi
    fi
    set -e
  fi
...
jq -n \
  ...
  '{source_branch: $branch, base_ref: $base, run_dir: $run_dir, projects: $projects, errors: $errors}'

if [[ "${need_terminal}" -eq 1 ]]; then
  echo "GLAB_RUN_IN_USER_TERMINAL=1" >&2
  echo "Rode no seu Terminal:" >&2
  echo "  ${SCRIPT_DIR}/glab-phase3-diff-bundle.sh $(printf '%q ' "${BRANCH}" "${REPOS[@]}")" >&2
  exit 2
fi
